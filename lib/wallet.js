/* Wallet connection for AI Runner. EIP-6963 discovery with a window.ethereum fallback, Robinhood Chain only.
...
function onAccounts(a) { W.address = a && a[0] ? a[0] : null; if (!W.address) { try { localStorage.removeItem('airun:wallet'); } catch {} } emit(); }
...
export async function connect(id) {
  ...
  try { localStorage.setItem('airun:wallet', d ? d.info.rdns : 'injected'); } catch {}
  emit();
  return state();
}

export async function restore() {
  let rdns = null; try { rdns = localStorage.getItem('airun:wallet'); } catch {}
...
export function disconnect() {
  W.address = null; try { localStorage.removeItem('airun:wallet'); } catch {}
  emit();
}
