/* Wallet connection for AI Labyrinth. EIP-6963 discovery with a window.ethereum fallback, Robinhood Chain only.
...
function onAccounts(a) { W.address = a && a[0] ? a[0] : null; if (!W.address) { try { localStorage.removeItem('aimaze:wallet'); } catch {} } emit(); }
...
export async function connect(id) {
  ...
  try { localStorage.setItem('aimaze:wallet', d ? d.info.rdns : 'injected'); } catch {}
  emit();
  return state();
}

export async function restore() {
  let rdns = null; try { rdns = localStorage.getItem('aimaze:wallet'); } catch {}
...
export function disconnect() {
  W.address = null; try { localStorage.removeItem('aimaze:wallet'); } catch {}
  emit();
}
