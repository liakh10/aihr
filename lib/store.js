/* Storage: Upstash Redis provisioned through the Vercel Marketplace.
   AIRUN_MEMORY=1 is only for the local test server; production never sets it. */
import { Redis } from '@upstash/redis';
let client = null;
export function redis() {
  if (client) return client;
  if (process.env.AIRUN_MEMORY === '1') return (client = memory());
...
export async function withLocks(keys, fn) {
  ...
  try {
    for (const k of keys) {
      ...
      if (!ok) throw Error('AI Runner is busy, try again.');
      held.push(k);
    }
    return await fn();
  } finally {
