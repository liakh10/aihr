/* Storage: Upstash Redis provisioned through the Vercel Marketplace.
   AIMAZE_MEMORY=1 is only for the local test server; production never sets it. */
import { Redis } from '@upstash/redis';
let client = null;
export function redis() {
  if (client) return client;
  if (process.env.AIMAZE_MEMORY === '1') return (client = memory());
  ...
  try {
    for (const k of keys) {
      ...
      for (let i = 0; i < 60 && !ok; i++) {
        if (await R.set('am:lock:' + k, tok, { nx: true, px: 5000 })) ok = true;
        else await new Promise(r => setTimeout(r, 80));
      }
      if (!ok) throw Error('AI Labyrinth is busy, try again.');
      held.push(k);
    }
    return await fn();
  } finally {
    for (const k of held) { try { if ((await R.get('am:lock:' + k)) === tok) await R.del('am:lock:' + k); } catch {} }
  }
}
function memory() {
