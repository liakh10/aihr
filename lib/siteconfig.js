...
export async function siteConfig(req) {
  ...
  const value = {
    factory: valid(process.env.AIMAZE_LABYRINTH) || pick('AIMAZE_LABYRINTH'),
    burner: valid(process.env.AIMAZE_BURNER) || pick('AIMAZE_BURNER'),
    buyback: valid(process.env.AIMAZE_UNUSED) || pick('AIMAZE_UNUSED'),
    ca: valid(process.env.AIMAZE_CA) || ca,
    origin
  };
