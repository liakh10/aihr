...
export async function siteConfig(req) {
  ...
  const value = {
    factory: valid(process.env.AIHR_LABYRINTH) || pick('AIHR_LABYRINTH'),
    burner: valid(process.env.AIHR_BURNER) || pick('AIHR_BURNER'),
    buyback: valid(process.env.AIHR_UNUSED) || pick('AIHR_UNUSED'),
    ca: valid(process.env.AIHR_CA) || ca,
    origin
  };
