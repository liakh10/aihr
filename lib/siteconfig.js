...
export async function siteConfig(req) {
  ...
  const value = {
    factory: valid(process.env.AIRUN_LABYRINTH) || pick('AIRUN_LABYRINTH'),
    burner: valid(process.env.AIRUN_BURNER) || pick('AIRUN_BURNER'),
    buyback: valid(process.env.AIRUN_UNUSED) || pick('AIRUN_UNUSED'),
    ca: valid(process.env.AIRUN_CA) || ca,
    origin
  };
